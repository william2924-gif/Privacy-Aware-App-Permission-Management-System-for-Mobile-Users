...
public class DetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txtPackageName =
                findViewById(R.id.txtPackageName);
        txtRiskLevel =
                findViewById(R.id.txtRiskLevel);
        txtPermissionCount =
                findViewById(R.id.txtPermissionCount);
        txtPrivacyScore =
                findViewById(R.id.txtPrivacyScore);
        txtPermissions =
                findViewById(R.id.txtPermissions);
        txtPrivacySummary =
                findViewById(R.id.txtPrivacySummary);
        txtRecommendation =
                findViewById(R.id.txtRecommendation);
        txtScanTime =
                findViewById(R.id.txtScanTime);
        progressPrivacy =
                findViewById(R.id.progressPrivacy);
    ...
    private void showApplication(AppInfo app) {
        txtAppName.setText(
                app.getAppName());
        ...
        txtPrivacyScore.setText(
                app.getPrivacyScore() +
                        " / 100");

        progressPrivacy.setProgressCompat(
                app.getPrivacyScore(),
                false);
    ...
    private void applyRiskStyle(String level) {
        ...
        if ("High".equalsIgnoreCase(level)) {
            txtRiskLevel.setBackgroundResource(
                    R.drawable.bg_risk_high);
            txtRiskLevel.setTextColor(
    ...
    private String generateRecommendation(
        ...
        if (permissions.contains("CAMERA")) {
            builder.append(
                    "• Disable Camera access when it is not required.\n\n");
        }

        if (permissions.contains("LOCATION")) {
            builder.append(
                    "• Prefer “Allow only while using the app” for Location.\n\n");
        }

        if (permissions.contains("CONTACTS")) {
            builder.append(
                    "• Confirm that Contacts access is essential to the app’s purpose.\n\n");
        }

        if (permissions.contains("SMS")) {
            builder.append(
                    "• SMS access can expose personal messages and verification codes.\n\n");
        }

        if (permissions.contains("PHONE")) {
            builder.append(
                    "• Grant Phone access only to applications you trust.\n\n");
        }

        if (permissions.contains("RECORD_AUDIO")) {
            builder.append(
                    "• Disable Microphone access unless voice recording is needed.\n\n");
        }

        if (builder.length() == 0) {
            builder.append(
                    "No significant sensitive permissions were detected.\n\n");
        }

        if (app.getPrivacyScore() >= 80) {
            builder.append(
                    "Overall: Low privacy risk. Continue reviewing permissions after app updates.");

        } else if (app.getPrivacyScore() >= 50) {
            builder.append(
                    "Overall: Review each sensitive permission and disable unnecessary access.");

        } else {
            builder.append(
                    "Overall: High attention required. Review all permissions before continuing to use this app.");
        }
